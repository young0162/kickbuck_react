...
class SoloItem extends Component {
  ...
  withCountUp = () => {
    var url =
      "http://localhost:9000/controller/bucketwithup?num=" + this.props.idx.num;
    Axios.get(url)
      .then(resData => {})
      .catch(error => {
  ...
  likeCountUp = () => {
    var url =
      "http://localhost:9000/controller/bucketlikeup?num=" + this.props.idx.num;
    Axios.get(url)
      .then(resData => {})
      .catch(error => {
        console.log("update error" + error);
